...
export default function Dashboard() {
  ...
  const requestAiMitigation = async (ticketId) => {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401 || response.status === 403) {
          throw new Error(`Accesso Negato (${response.status}): Token non valido o scaduto.`);
        }
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 py-8">
        ...
        {loadingAssets ? (
          <div className="text-center py-16">
            ...
            <p className="text-[#555b57] font-mono text-sm">Caricamento inventario...</p>
          </div>
        ) : assets.length === 0 ? (
          ...
          <div className="flex flex-col gap-4">
            {assets.map((asset) => (
              <div key={asset.id} className="group rounded-xl overflow-hidden transition-all duration-500"
                style={cardStyle}
                onMouseEnter={(e) => {
                  e.currentTarget.style.borderColor = asset.tipo === 'SERVER' ? 'rgba(127,173,144,0.15)' : 'rgba(139,165,196,0.15)';
        ...
        {aiResponse && (
          <div className="mt-8 rounded-xl overflow-hidden" style={cardStyle}>
